...
test('should measure sym shift', () => {
  ...
  // asym multiplets
  expect(measureSymShift(Float64Array.from([0, 1, 0, 1, 0]))).toBe(0);
  expect(measureSymShift(Float64Array.from([0, 1, 0, 0.8, 0]), 30)).toBe(2);
  expect(measureSymShift(Float64Array.from([0, 1, 0, 0.8, 0]), 95)).toBe(0);
});
